...
export default function TenantShell({ children, breadcrumb, activeModule, onModuleClick }: Props) {
  ...
  return (
    <div className="min-h-screen flex" style={{ background: BG, fontFamily: "'Sarabun', sans-serif" }}>
      ...
      <div className={`shrink-0 flex transition-all duration-300 ${sidebarExpanded ? "w-[200px]" : "w-[52px]"}`} style={{ borderRight: `1px solid ${BORDER}` }}>
        {/* Icon Bar (always visible) */}
        <div className={`shrink-0 flex flex-col items-center py-3 gap-1 z-50 transition-all duration-300 ${sidebarExpanded ? "w-full" : "w-[52px]"}`} style={{ background: "#FFFFFF" }}>
          {/* JIGSAW logo */}
